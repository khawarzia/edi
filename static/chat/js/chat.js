...
const userDiv = (senderId, receiverId, name, online, profile_pic) =>
    (`<a href="/chat/${senderId}/${receiverId}" id="user${receiverId}" class="collection-item row">
                    <img src="${profile_pic}" class="col s2">
                    <div class="col s10">
...
function getUsers(senderId, callback) {
    return $.get('/api/users', function (data) {
        if (userState !== JSON.stringify(data)) {
            ...
            const doc = data.reduce((res, infor) => {
                if (infor.id === senderId) {
                    return res
                } else {
                    return [userDiv(senderId, infor.id, infor.username, infor.online, infor.profileurl), ...res]
                }
            }, [])
            callback(doc)
        }
